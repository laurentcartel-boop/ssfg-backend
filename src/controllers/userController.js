...
async function listUsers(req, res) {
  try {
    ...
    const users = await User.findAll({
      where,
      include: [{ model: Club, as: 'club', attributes: ['id', 'code', 'name', 'short_name'], required: false }],
    ...
    res.json({
      users: users.map((u) => {
        ...
        j.club = u.club ? { id: u.club.id, code: u.club.code, name: u.club.name, short_name: u.club.short_name } : null;
        return j;
      }),
    });
  } catch (err) {
...
async function getUser(req, res) {
  try {
    const user = await User.findByPk(req.params.id, {
      attributes: { exclude: ['password_hash'] },
      include: [{ model: IndexHistory, as: 'indexHistory', limit: 50, order: [['created_at', 'DESC']] }],
    });
...
async function updateUser(req, res) {
  try {
    ...
    const isClubAdmin = req.user.role === 'admin' && req.user.club_id && user.club_id === req.user.club_id;
    ...
    if (allowed.index_value !== undefined && Number(allowed.index_value) !== prevIndex) {
      try {
    ...
    res.json({ user: user.toSafeJSON(), message: 'Profil mis à jour' });
  } catch (err) {
...
async function listClubs(req, res) {
  try {
    if (!Club) return res.status(500).json({ error: 'Modèle Club absent' });
    let clubs = await Club.findAll({ order: [['sort_order', 'ASC'], ['name', 'ASC']] });
    clubs = clubs.filter((c) => c.is_active !== false);
    res.json({ clubs: clubs.map((c) => ({ id: c.id, code: c.code, name: c.name, short_name: c.short_name })) });
  } catch (err) {
...
async function deleteUser(req, res) {
  try {
    if (!['platine_admin', 'super_admin'].includes(req.user.role)) {
      return res.status(403).json({ error: 'Seul un AdminPlatine peut supprimer un joueur' });
    }
    ...
    if (user.role === 'platine_admin' && req.user.role !== 'platine_admin') {
      return res.status(403).json({ error: 'Impossible de supprimer un AdminPlatine' });
    }
    ...
    if (played > 0) {
      ...
      return res.json({ message: `${user.first_name} ${user.last_name} désactivé (${played} partie(s) conservées)`, soft: true });
    }
    await user.destroy();
...
module.exports = { listClubs, listUsers, getUser, updateUser, deleteUser };
